...
static void mark(const char *where, uint32_t *p, uint32_t *e) {
    ...
    for(uint32_t *i = p; i <= e; i++) { /// scanning 4 byte words ex)  p, p+4, p+8, etc. 
        uint32_t val = *i;
        hdr_t * h = is_ptr(val);
        if (!h) {
            continue;
        ...
        if (val == (uint32_t)ck_data_start(h)) { // if it's a ptr to start, increase counter
            h->refs_start++;
        } else { // if it's pointing to middle, increment that.
            h->refs_middle++;
        }
        if (!h->mark) { // if not marked, mark and sweep through payload
            h->mark = 1;
            uint32_t *sp = (uint32_t*)ck_data_start(h);
            uint32_t *ep = (uint32_t*)ck_data_end(h) - 1;
            if(sp <= ep)
                mark("inside block", sp, ep);
        }
...
static unsigned sweep_leak(int warn_no_start_ref_p) {
    ...
    for(hdr_t *h = ck_first_alloc(); h; h = ck_next_hdr(h), nblocks++) {
        if (!h->mark) {
            ...
            ck_error(h, "GC:DEFINITE LEAK of block=%u [addr=%p]\n",
                h->block_id, h);
        } else if (warn_no_start_ref_p && !h->refs_start) {
            maybe_errors++;
            ck_error(h, "GC:MAYBE LEAK of block %u [addr=%p] (no pointer to the start)\n", h->block_id, h);   
        }
        // nblocks++;
...
static void mark_all(uint32_t *sp) {
    ...
    mark("data", __data_start__, __data_end__);
